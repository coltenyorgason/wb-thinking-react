...
const addRow = async () => {
   ...
   const response = await axios.post('/addInvoice', { description: 'Job description here' })
    ...
    //     // creat a new "blank" object for the new row (modeled after each element in TEST_Data)
    //     const newRow = {
    //         id: globalId,
    ...
    //    // all above (everything except new row creation) can be done with this instead
    ...
    const deleteRow = async (itemId) => {
        ...
        if (!response.data.error) { // if the response did not throw an error
            ...
            //using the given id as above, find the corresponding element in currentData and remove it
